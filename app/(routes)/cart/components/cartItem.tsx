...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const cartItems = useSelector<RootState,Product[]>(state=> state.cartCounterSlice.items)
  ...
  const addToCart=(event:React.MouseEvent<SVGSVGElement,MouseEvent>)=>{
      ...
      dispatch(addItem({...data,cartCount:(parseInt(prod.length>0?prod[0].cartCount:"0")+1).toString()}));
    ...
    const removeOnce =(event:React.MouseEvent<SVGSVGElement,MouseEvent>)=>{
      ...
      dispatch(removeItemOnce({...data,cartCount:(parseInt(prod.length>0?prod[0].cartCount:"0")-1).toString()}));
      ...
      <div className="relative ml-4 flex flex-1 flex-col justify-between sm:ml-6">
        ...
        <div className="relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0">
          ...
          <div className="mt-1 flex text-sm">
            ...
            {/* <p className="ml-4 border-l border-gray-200 pl-4 text-gray-500">{data.cartCount}</p> */}
          </div>
